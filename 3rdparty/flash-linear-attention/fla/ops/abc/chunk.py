...
if __name__ == '__main__':
    ...
    # q0 = torch.where(torch.rand_like(q0) < 0.1, q0 * 2, q0)
    # k0 = torch.where(torch.rand_like(k0) < 0.1, k0 * 2, k0)
    # v0 = torch.where(torch.rand_like(v0) < 0.1, v0 * 2, v0)
    # s0 = torch.where(torch.rand_like(s0) < 0.1, s0 * 2, s0).clamp_(-32, 32)
    print('Testing...')
    ...
    ref, _ = naive_recurrent_abc(q1, k1, v1, s1)
    ref.backward(do)
    ...
    @triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            line_vals=['gate', 'chunk', 'gate_bwd', 'chunk_bwd'],
            # label name for the lines
            line_names=['gate', 'chunk', 'gate_bwd', 'chunk_bwd'],
            # line styles
    ...
    def benchmark(seq_len, provider):
        ...
        dtype = torch.bfloat16
        requires_grad = True
        batch_size, n_heads, d_head, n_slots = 16, 4, 128, 128
        ...
        if provider == 'gate_bwd':
            results = triton.testing.do_bench(lambda: gated_chunk_abc(q, k, v, s)[0].backward(do), quantiles=quantiles)
