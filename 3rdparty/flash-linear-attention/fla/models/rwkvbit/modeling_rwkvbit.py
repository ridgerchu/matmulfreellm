...
class RwkvBitFeedForward(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        self.key = BitLinear(hidden_size, intermediate_size * 2, bias=False)
    ...
    def forward(self, hidden, state=None):
        ...
        key = self.key(key)
        gate, key = key.chunk(2, -1)
        return swiglu_linear(gate, key, self.value.weight, self.value.bias), state
        # receptance = torch.sigmoid(self.receptance(receptance))
