...
@triton.jit
def flash_cross_entropy_fwd_kernel(
    loss_ptr,  # data ptrs
    lse_ptr,
    logits_ptr,
    labels_ptr,
...
@triton.jit
def flash_cross_entropy_bwd_kernel(
...
class FlashCrossEntropyLossFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        logits,
        ...
        with torch.cuda.device(logits.device.index):
            flash_cross_entropy_fwd_kernel[(n_rows, n_splits)](
                losses,  # data ptrs
                lse,
                logits,
                labels,
    ...
    def backward(ctx, grad_losses):
        ...
        # ValueError: Pointer argument (at 0) cannot be accessed from Triton (cpu tensor?)
        with torch.cuda.device(logits.device.index):
            flash_cross_entropy_bwd_kernel[grid](
                dlogits,  # data ptrs
                grad_losses,
                logits,
                lse,
...
def cross_entropy_loss(
    ...
    Returns:
        losses: (batch,), float
    """
    return FlashCrossEntropyLossFunction.apply(
        logits,
        labels,
        label_smoothing,
        lse_square_scale,
...
class FlashCrossEntropyLoss(nn.Module):
    def __init__(
        self,
        ignore_index=-100,
        reduction="mean",
...
if __name__ == "__main__":
    naive_cross_entropy = nn.CrossEntropyLoss()
    flash_cross_entropy = FlashCrossEntropyLoss()
    batch = 4096
    logits = torch.randn(batch, 32000, device='cuda')
    target = torch.randint(0, 32000, (batch,), device='cuda')
    assert naive_cross_entropy(logits, target).allclose(flash_cross_entropy(logits, target)), breakpoint()
    ...
    def benchmark(batch, provider):
        ...
        logits = torch.randn(batch, n_vocab, requires_grad=requires_grad, dtype=dtype, device=device)
        target = torch.randint(0, n_vocab, (batch,), device=device)
        dl = torch.tensor(1., dtype=dtype, device=device)
        quantiles = [0.5, 0.2, 0.8]
        if provider == 'naive':
            results = triton.testing.do_bench(lambda: naive_cross_entropy(logits, target), quantiles=quantiles)
        elif provider == 'flash':
            results = triton.testing.do_bench(lambda: flash_cross_entropy(logits, target), quantiles=quantiles)
        elif provider == 'naive_bwd':
            results = triton.testing.do_bench(lambda: naive_cross_entropy(logits, target).backward(dl), quantiles=quantiles)
        elif provider == 'flash_bwd':
            results = triton.testing.do_bench(lambda: flash_cross_entropy(logits, target).backward(dl), quantiles=quantiles)
        return results
