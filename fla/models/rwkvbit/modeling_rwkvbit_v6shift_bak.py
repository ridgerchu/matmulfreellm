...
from fla.ops.bitnet import BitLinear, BitLinear_wonorm, BitLinear_wonorm_bmm
...
class RwkvBitSelfAttention(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        TIME_MIX_EXTRA_DIM = 32  # generate TIME_MIX for k,v,r
        self.time_maa_w1 = nn.Parameter(torch.zeros(hidden_size, TIME_MIX_EXTRA_DIM * 3).uniform_(-1e-4, 1e-4).transpose(0, 1))
        self.time_maa_linear_w1 = BitLinear_wonorm(hidden_size, TIME_MIX_EXTRA_DIM * 3)
        self.time_maa_linear_w1.weight = self.time_maa_w1
        self.time_maa_w2 = nn.Parameter(torch.zeros(3, TIME_MIX_EXTRA_DIM, hidden_size).uniform_(-1e-4, 1e-4))
        self.time_maa_linear_w2 = BitLinear_wonorm_bmm(hidden_size, TIME_MIX_EXTRA_DIM * 3)
        self.time_maa_linear_w2.weight = self.time_maa_w2
    ...
    def extract_key_value(self, hidden, state=None):
        ...
        shifted = self.time_shift(hidden) - hidden

        shifed_other = torch.tanh(self.time_maa_linear_w1(hidden)).view(B * T, 3, -1).transpose(0, 1)
        shifed_other = self.time_maa_linear_w2(shifed_other).view(3, B, T, -1)
        mk, mv, mr = shifed_other.unbind(dim=0)

        key = hidden + shifted * (self.time_mix_key + mk)
        value = hidden + shifted * (self.time_mix_value + mv)
        receptance = hidden + shifted * (self.time_mix_receptance + mr)
